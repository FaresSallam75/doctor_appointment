...
class _ScheduleTabState extends State<ScheduleTab> {
  ...
  void initState() {
    ...
    context.read<PaymentCubit>().viewPaymentsData();
            ...
            BlocBuilder<AppointmentCubit, AppointmentState>(
              builder: (context, state) {
                if (state is AppointmentStateLoading) {
                  ...
                  context.read<DoctorCubit>().listRatingDoctors.where((check) {
                    getUserId = check['user_id'].toString();
                    getDoctorId = check['doctor_id'].toString();
                    return true; // or add your actual condition here
                  }).toList();

                  context.read<PaymentCubit>().listPayments.where((payments) {
                    paymentsUserId = payments['user_id'].toString();
                    paymentsdoctorId = payments['doctor_id'].toString();
                    return true;
                  }).toList();
                                  ...
                                  children: [
                                    ...
                                    selectedIndex == 1
                                        ? CustomElvatedButton(
                                          text: "Pay",
                                          onPressedDelete: () {
                                            if (paymentsUserId.toString() ==
                                                    myBox!
                                                        .get("userId")
                                                        .toString() &&
                                                paymentsdoctorId.toString() ==
                                                    appointment.doctorId
                                                        .toString()) {
                                              ScaffoldMessenger.of(
                                                ...
                                                SnackBar(
                                                  content: Text(
                                                    "You Already Paid For This Doctor .",
                                                  ),
                                                ),
                                              );
                                            } else {
